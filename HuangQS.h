...
void median_filter_3D(int *data, int *dims, int *out, int *fmin, int *fsiz, int *fmax){
  ...
  for(;;){
    ...
    if(++iteration % 1000 == 0 || iteration+1 == total){
      clock_gettime(CLOCK_MONOTONIC, &spec);
      float p = ((float)iteration)/total;
      ...
      if(sec > TIMELIMIT || iteration+1 == total){
        ...
        // printf("Took %.3fs\n", sec);
        float e = (1.f/p)*sec;
        printf("huangQS\twindow,time,total\t%d\t%.3f\t%.3f\n",WINDOW_SIZE,sec,e);
    ...
    // verbose = 0;
    // printf("Med\n");
    out[ind] = quick_select(window, N*N*N);
